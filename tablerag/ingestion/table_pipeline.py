...
async def parse_table_region(crop_png: bytes, grid: list[list[str | None]] | None,
    ...
    parse = await parser.parse_table(
        crop_png, TableCtx(locale_hint=locale or "unknown"))
...
_SUMMARY_PROMPT = (
    ...
    "row/column dimensions, measures, period and units if visible. Reply in "
    "the language of the table content, nothing else.\n\n{html}")


async def summarize_table(html: str) -> str | None:
    ...
    try:
        ...
        async for token in chat.chat(
                [Msg(role="user",
                     content=_SUMMARY_PROMPT.format(html=html[:_SUMMARY_HTML_LIMIT]))],
