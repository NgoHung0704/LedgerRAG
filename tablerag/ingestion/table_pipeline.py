...
def build_summary_prompt(html: str, locale: str | None,
    ...
    prompt = _SUMMARY_PROMPT.format(language_rule=language_rule,
                                    html=html[:_SUMMARY_HTML_LIMIT])
    if context:
        # The summary is what routes a question to this table, and the words a
        # reader would use are often only in the heading ABOVE it: a booklet's
        # tax grid is asked for as "la fiscalité du PERECOL", which appears
        # nowhere in its own cells.
        prompt += (f"\n\nThe page prints this heading above the table: "
                   f"{context}\nName that subject in the summary.")
    return prompt


async def summarize_table(html: str, locale: str | None = None,
    ...
    try:
        ...
        async for token in chat.chat(
                [Msg(role="user", content=build_summary_prompt(html, locale, context))],
