...
| 4 | Hybrid retrieval + answer verification | **Code complete** — answer-number verification (locale-aware + arithmetic whitelist, UI badges); hybrid dense+sparse retrieval (local lexical sparse + Qdrant server-side IDF, native RRF fusion, graceful dense-only fallback on unmigrated collections); Rerank step (reranker role, honest degradation when disabled/down); `make eval-qa` harness. **To activate on a deployment:** run the migration `docker compose exec api python -m tablerag.scripts.reindex_all`, then fill `tests/eval/qa/questions.jsonl` with ≥30 real Q&A pairs and run the gate. |
