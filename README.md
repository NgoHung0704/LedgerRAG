...
| 2 | Table sub-pipeline (three representations) | **Measured on the box (first production-path run, 2026-07-20): `make eval-tables` = 88.4%** — above the Phase 0 direct-drive band (84–90%) after the duplicate-dimensions contract (a dropped header level leaves two records with identical dims; the retry names the offending pair and recovers the table — pivot_de_umsatz 0/16 → 16/16). The **≥95% gate is not met with `qwen3-vl:8b-instruct`**: the remaining misses are deep-pivot sub-row misattribution (unique-but-wrong labels, no structural fingerprint — same family as the Phase 3 finding, SPEC §7). Parser-model upgrade is the lever (C3 makes the swap config-only). **Never edit the parser prompt without an isolated eval-tables A/B: wording changes flip unrelated tables between 0% and 100%.** **That 88.4% grades TRANSCRIPTION of tables already cropped for the gate — it never asks whether the platform would find them on a page. `make eval-detection` does, on real customer factsheets. It read **25% (3/12 pages)** on 2026-08-13: reading a table that was found worked 88% of the time, finding it on a real page worked 25% of the time. `find_tables` returned either one page-wide blob or nothing as soon as a table shared a page with anything else. It reads **100% (12/12) since 2026-08-13** through a detector built on word coordinates (`ingestion/word_tables.py`): columns are the x positions that repeat down a run, page columns are separated by the strips few lines reach across, stacked tables are separated by their row pitch, and any region whose boundaries land inside a number is refused whoever drew it. Both numbers are true and only together are they honest — a corpus can be read at 88% and still be unreachable.** **DO NOT REPROCESS EXISTING DOCUMENTS ON THAT 100%.** It is measured on six fund factsheets, and two defects found while reviewing it (2026-08-17) are open and reproduced in `tests/unit/test_word_tables.py` as strict xfails: (1) a table whose FIRST COLUMN IS SHORT and whose column gaps are wide — `A | 1 | 21 700`, the commonest shape in the HR corpus — is banded into one strip per column, and either vanishes or, worse, is filed WITHOUT its label column, as figures with no dimension; (2) `cuts_through_numbers` now applies to ruled regions too, so one cell holding `0,90 1,40` — two columns fused by a hairline that did not print, which `_TABLE_STRATEGIES` already documents as common in HR grids — drops the whole table, with no word-detector fallback because of defect (1). Nothing deployed is affected while documents are not reprocessed: every change is in ingestion and the stored elements are the old ones. The danger is the next reprocess or upload, whenever that happens.** |
