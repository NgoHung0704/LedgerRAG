...
| 4 | Hybrid retrieval + answer verification | **Answer verification done** — every number in an answer is cross-checked (locale-aware) against the cited sources, with an arithmetic whitelist; unmatched figures are badged in the UI. Per-KB / per-request toggle. **Remaining:** hybrid dense+sparse retrieval + reranker + the `eval-qa` gate (needs a real Q&A corpus). |
