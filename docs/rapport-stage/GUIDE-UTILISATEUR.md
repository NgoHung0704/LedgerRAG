...
**Version du guide :** 18/08/2026 · **Interface :** français, anglais, vietnamien,
...
5. [Vérifier et corriger une lecture](#5-vérifier-et-corriger-une-lecture)
6. [Les assistants](#6-les-assistants)
7. [Intégrer un assistant dans une autre application](#7-intégrer-un-assistant-dans-une-autre-application)
8. [Écrans d'administration](#8-écrans-dadministration)
9. [⚠️ Précautions importantes](#9--précautions-importantes)
...
| *Une source est signalée comme à vérifier* | Ce tableau attend une relecture humaine (§5). |
...
| Échec | Le message d'erreur est affiché sur la ligne. Voir §10. |

---

## 5. Vérifier et corriger une lecture

### 5.1 L'onglet **Révision**

Les tableaux que l'analyseur n'a pas su lire avec certitude arrivent ici. Pour
chacun : l'original à côté de la lecture qui en a été faite. Vous **validez**,
**corrigez** ou **mettez de côté**.

> Cette file est courte par construction : quand le système signale, il a raison
> dans la quasi-totalité des cas. En revanche, elle ne contient pas tout — un
> tableau mal lu avec assurance n'y apparaît pas. C'est pourquoi les sources
> restent toujours consultables.

### 5.2 L'inspecteur de document

Depuis la liste des documents, bouton **Inspecter**. Page par page, vous voyez ce
que le système a réellement extrait, et vous pouvez :

- **corriger un texte ou un tableau à la main** — la correction est ré-indexée
  immédiatement, les réponses suivantes s'en servent ;
- **relire un tableau plus attentivement** (« seconde lecture ») — le modèle
  reprend l'image du tableau, vous comparez et vous décidez ;
- **promouvoir un texte en tableau** quand une grille a été lue comme du texte ;
- **joindre deux tableaux** que la détection a séparés à tort ;
- **relire une page en colonnes** (diaporama converti) de façon structurée.

Rien n'est jamais écrasé sans qu'un humain l'ait vu.
...
## 7. Intégrer un assistant dans une autre application
...
| L'`<iframe>` reste **blanc** | `EMBED_FRAME_ANCESTORS` n'est pas renseigné, ou ne correspond pas exactement à l'origine de l'application hôte (protocole et port compris). Voir §7. |
| Une modification du `.env` **reste sans effet** | Une valeur enregistrée en base prend le dessus. **Fournisseurs de modèles** indique quelle couche a décidé (§8). |
