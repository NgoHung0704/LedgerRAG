...
export function Button({
  ...
  const styles = {
    ...
    secondary:
      "border border-slate-300 bg-white text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700",
    ghost:
...
export function Modal({
  ...
  return (
    ...
    >
      <div
        className={`max-h-[85vh] w-full ${wide ? "max-w-3xl" : "max-w-md"} overflow-y-auto rounded-xl bg-white p-5 shadow-xl dark:bg-[#171d24] dark:text-slate-200`}
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white py-14 text-center dark:border-slate-700 dark:bg-[#171d24]">
      ...
      <div className="mt-1 max-w-sm text-xs text-slate-400 dark:text-slate-500">{hint}</div>
    </div>
  );
}
