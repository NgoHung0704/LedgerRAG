  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className="flex h-7 w-7 items-center justify-center rounded border border-transparent text-slate-400 hover:border-slate-200 hover:bg-white hover:text-slate-700"
      >
        <MoreVertical size={16} />
      </button>

      {open && (
        <div className="absolute right-0 z-20 mt-1 w-60 rounded-lg border border-slate-200 bg-white p-1.5 shadow-md">
          {!confirming ? (
            <>
              <button
                type="button"
                onClick={() => router.push(`/kb/${kb.id}`)}
                className="flex w-full items-center gap-2.5 rounded px-2.5 py-2 text-left text-[13px] text-slate-700 hover:bg-slate-50"
              >
